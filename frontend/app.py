...
.cv-mobile-nav-marker{display:none}div[data-testid="stElementContainer"]:has(.cv-mobile-nav-marker),div[data-testid="stElementContainer"]:has(.cv-mobile-nav-marker)+div[data-testid="stElementContainer"]{display:none}
...
@media(max-width:640px){.block-container{padding:4.6rem .82rem 3.5rem}.cv-hero{min-height:auto;padding:1.4rem 0 1.25rem}.cv-hero h1{font-size:2.15rem;max-width:92%}.cv-hero p{font-size:.84rem;line-height:1.5}.cv-anatomy{right:-4.2rem;bottom:-.8rem;width:155px;opacity:.08}.cv-steps{grid-template-columns:1fr}.cv-step{border-right:0;border-bottom:1px solid var(--atlas-rule);padding:.58rem .72rem}.cv-step:last-child{border-bottom:0}.cv-note{box-shadow:5px 6px 0 #d8d1c3}.cv-profile{align-items:flex-start}.cv-profile .cv-avatar{width:50px;height:50px;flex:0 0 50px}.cv-profile h2{font-size:1.3rem}.cv-episode-head{align-items:flex-start}.cv-case-no{min-width:42px;font-size:1.75rem;padding-right:.55rem}.cv-episode-title{font-size:1rem}.cv-pill{font-size:.54rem}.cv-visit{padding-left:1rem}.cv-field span{font-size:.78rem}.cv-record-grid{grid-template-columns:1fr 1fr;gap:.4rem}.cv-record-fact{min-height:60px;padding:.5rem}.cv-soap-scroll{height:42vh;min-height:220px;padding:.75rem;font-size:.71rem}div.stButton>button,div.stLinkButton>a{min-height:2.6rem}.cv-brand{padding-bottom:1rem}div[data-testid="stElementContainer"]:has(.cv-mobile-nav-marker)+div[data-testid="stElementContainer"]{display:block;position:fixed;z-index:999999;top:6px;left:58px;right:9px;background:rgba(255,253,248,.97);border:1px solid #c9c3b6;box-shadow:0 5px 18px #10233020;backdrop-filter:blur(12px)}div[data-testid="stElementContainer"]:has(.cv-mobile-nav-marker)+div[data-testid="stElementContainer"] [role="radiogroup"]{display:grid;grid-template-columns:repeat(3,1fr);gap:0}div[data-testid="stElementContainer"]:has(.cv-mobile-nav-marker)+div[data-testid="stElementContainer"] label[data-baseweb="radio"]{display:flex;justify-content:center;min-height:46px;padding:.5rem;border-right:1px solid #d7d1c5;background:transparent}div[data-testid="stElementContainer"]:has(.cv-mobile-nav-marker)+div[data-testid="stElementContainer"] label[data-baseweb="radio"]:last-child{border-right:0}div[data-testid="stElementContainer"]:has(.cv-mobile-nav-marker)+div[data-testid="stElementContainer"] label[data-baseweb="radio"]>div:first-child{display:none}div[data-testid="stElementContainer"]:has(.cv-mobile-nav-marker)+div[data-testid="stElementContainer"] label[data-baseweb="radio"] p{font-size:1.2rem;line-height:1;color:var(--atlas-ink)}div[data-testid="stElementContainer"]:has(.cv-mobile-nav-marker)+div[data-testid="stElementContainer"] label[data-baseweb="radio"]:has(input:checked){background:#e8efef;box-shadow:inset 0 -3px 0 var(--atlas-coral)}}
...
def mobile_navigation(page:str):
    icon_to_page={"＋":"＋  Case intake","📖":"◫  Case registry","⚙":"⚙  Archive control"};page_to_icon={value:key for key,value in icon_to_page.items()}
    expected=page_to_icon.get(page,"📖")
    if st.session_state.get("mobile_nav") not in icon_to_page or icon_to_page.get(st.session_state.get("mobile_nav"))!=page:
        st.session_state.mobile_nav=expected
    def change_page():st.session_state.nav_page=icon_to_page[st.session_state.mobile_nav]
    st.markdown('<div class="cv-mobile-nav-marker"></div>',unsafe_allow_html=True)
    st.radio("Mobile navigation",list(icon_to_page),horizontal=True,key="mobile_nav",on_change=change_page,label_visibility="collapsed")
...
def sidebar():
    ...
    if not EMBEDDED_MODE or st.session_state.get("app_user"):mobile_navigation(page)
    return page
