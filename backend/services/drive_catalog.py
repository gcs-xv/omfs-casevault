...
def patient_from_folder(item:dict)->dict:
    ...
    remaining=[p for p in parts[1:] if p.upper() not in {"L","P"} and not re.fullmatch(r"(?i)\d+\s*(Tahun|Bulan|Hari)",p) and not re.match(r"(?i)^RM\b",p)]
    hospital=remaining[-1] if remaining else ""
    insurance=" - ".join(remaining[:-1]) if len(remaining)>1 else ""
    return {"id":item.get("id"),"name":name,"title":honorific,"rm":rm,"rm_normalized":normalize_rm(rm),"sex":sex,"age":age,"age_unit":age_unit,"insurance":insurance,"hospital":hospital,"folder_name":folder_name,"drive_url":item.get("webViewLink") or f"https://drive.google.com/drive/folders/{item.get('id')}"}
